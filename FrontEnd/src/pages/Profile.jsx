...
import { Grid, Avatar, List, Typography, Container, Card, Divider, Box, Tabs, Tab } from "@mui/material/";
...
function Profile() {
  ...
  useEffect(() => {
    fetchUserData("fengqi").then(resp => {
      if (resp.status == 200) {
        resp.json().then(data => {
      ...
      <Container>
        <Grid container spacing={3} alignItems="center" justifyContent="space-around">
          ...
          <Grid item xs={6} md={5} alignItems="center" justifyItems="center">
            <Card sx={{px:2, py:1}} style={{backgroundColor:"#cfd8dc"}} variant="outlined">
            <List>
